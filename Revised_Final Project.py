...
# -*- coding: utf-8 -*-
"""
Varner's Greenhouse & Nursery - 2025 Fall Wholesale Order Form (improved)
Python/Tkinter desktop app

Improvements:
- trace_add / trace compatibility so live updates work on more Python/Tk versions
- canvas/frame width binding so the product list resizes with window
- safer entry-style application (peach background)
- small defensive guards for parsing fields
- initial focus on first entry for quicker data entry
"""
...
def add_trace(var, mode, callback):
    """Add a trace handler that works with both trace_add and trace (compat)."""
    try:
        # Python 3.6+: trace_add
...
class OrderForm(tk.Tk):
    def __init__(self):
        ...
        # Frame that will contain products
        ...
        self._canvas_window = canvas.create_window((0,0), window=self.product_frame, anchor="nw")

        # Make product_frame width track canvas width on resize
        ...
        # Apply peach style to entry/text widgets
        self.style_entries_peach(self)

        # Recompute when relevant fields change
        self.bind_variable_updates()

        # focus first entry for quicker input
        try:
            self.business_name.focus_set()
        except Exception:
            pass
    ...
    def build_header(self):
        ...
        self.business_name = tk.Entry(frm)
        ...
        self.contact_name = tk.Entry(frm)
        ...
        self.business_phone = tk.Entry(frm)
        ...
        for txt in ("PAYS SALES TAX", "SALES TAX EXEMPT"):
            ttk.Radiobutton(tax_frame, text=txt, value=txt,
                            variable=self.sales_tax_status, command=self.recompute_totals).pack(side="left", padx=6)

        ttk.Label(frm, text="Sales Tax Rate:").grid(row=3, column=2, sticky="e", padx=6, pady=4)
        self.tax_rate_entry = tk.Entry(frm, width=8)
        ...
        ttk.Radiobutton(fulfill_frame, text="DELIVERY", value="DELIVERY",
                        variable=self.delivery_selected, command=self.on_delivery_toggle).pack(side="left", padx=6)
        ttk.Radiobutton(fulfill_frame, text="PICK UP", value="PICK UP",
                        variable=self.delivery_selected, command=self.on_delivery_toggle).pack(side="left", padx=6)

        ttk.Label(frm, text="Delivery Fee:").grid(row=4, column=2, sticky="e", padx=6, pady=4)
        self.delivery_fee_entry = tk.Entry(frm, width=10)
        ...
        self.notes = tk.Text(frm, height=3, width=60, wrap="word")
    ...
    def build_table(self, parent, items_dict, product_info, vcmd, subtitle=None):
        ...
        for name, _ in items_dict.items():
            ...
            # Live update on change (trace_add / trace compatibility)
            ...
            # register trace safely
            add_trace(v, "write", make_cb())
    ...
    def style_entries_peach(widget):
        # Walk children and set bg where supported
        for child in widget.winfo_children():
            try:
                if isinstance(child, (tk.Entry, tk.Text)):
                    child.configure(bg=PEACH)
            except Exception:
                pass
            # Recurse
